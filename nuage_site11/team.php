...
<div id="applyModal" style="display:none;position:fixed;inset:0;z-index:9999;align-items:center;justify-content:center;background:rgba(0,0,0,.6);">
  <div style="background:#fff;border-radius:16px;max-width:480px;width:90%;padding:1.5rem;box-shadow:0 20px 50px rgba(0,0,0,0.25);">
    <h2 style="margin-top:0;color:#002D72;">Apply for Employment</h2>
    <form method="POST">
      <label>Name</label>
      <input type="text" name="full_name" required style="width:100%;padding:10px;margin-bottom:10px;">
      ...
      <textarea name="message" rows="3" style="width:100%;padding:10px;margin-bottom:10px;"></textarea>
      <div style="display:flex;justify-content:flex-end;gap:10px;">
        <button type="button" onclick="document.getElementById('applyModal').style.display='none'" 
                style="background:#ccc;padding:8px 14px;border:none;border-radius:6px;cursor:pointer;">Cancel</button>
        <button type="submit" name="apply_submit" 
                style="background:#EB1F48;color:#fff;padding:8px 16px;border:none;border-radius:6px;cursor:pointer;">Submit</button>
      </div>
    </form>
  </div>
</div>
...
<div style="text-align:center;margin:2rem;">
  <button onclick="openApplyModal()" style="background:#EB1F48;color:#fff;font-weight:600;border:none;border-radius:10px;padding:12px 22px;cursor:pointer;">
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['apply_submit'])) {
    $to = 'info@nuagefitness-studio.com';
    $subject = 'New Employment Application — ' . htmlspecialchars($_POST['position']);
    $body = "A new employment application has been submitted:\n\n" .
    ...
    $headers = "From: NuAge Fitness Studio <no-reply@nuagefitness-studio.com>\r\n" .
    ...
    if (mail($to, $subject, $body, $headers)) {
        ...
        echo "<script>alert('There was an issue sending your application. Please try again.');</script>";
    }
}
?>
